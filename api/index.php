...
<head>
    ...
    <style>
        ...
        html, body {
            ...
            overflow: hidden; /* Mengunci total layar luar agar tidak goyang liar */
        ...
        /* --- KONTEN FORM ATAS (DENGAN TRANSISI HALUS) --- */
        .konten-form {
            width: 100%;
            display: flex;
            flex-direction: column;
            align-items: center;
            box-sizing: border-box;
            transition: transform 0.2s ease-out; /* Mengontrol pergeseran konten */
        ...
        /* --- CONTAINER TOMBOL MERAH FIXED MASIH MELAYANG --- */
        .tombol-container {
            width: 100%;
            max-width: 400px;
            padding: 15px 20px;
...
<body>
    ...
    <script>
        ...
        // --- SOLUSI PENYETELAN JARAK KONTEN VS KEYBOARD ---
            ...
            window.visualViewport.addEventListener('resize', function() {
                ...
                document.querySelectorAll('.tombol-container').forEach(el => {
                    if (selisihKeyboard > 60) {
                        // 1. Tombol merah sukses melompat naik ke atas keyboard
                        el.style.bottom = selisihKeyboard + 'px';
                        
                        // 2. Turunkan seluruh posisi konten atas dikit agar pas berada di area tengah kosong (tidak terpotong ke atas)
                        document.getElementById('konten1').style.transform = 'translateY(15px)';
                        document.getElementById('konten2').style.transform = 'translateY(15px)';
                    } else {
                        // Kembali ke normal saat keyboard ditutup
                        el.style.bottom = '0px';
                        document.getElementById('konten1').style.transform = 'translateY(0px)';
                        document.getElementById('konten2').style.transform = 'translateY(0px)';
                    }
                });
            });
        }
